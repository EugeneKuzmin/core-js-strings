...
function getFirstChar(value) {
  if (value.length) return value.charAt();
  return 0;
}
...
function removeFirstOccurrences(str, value) {
  return str
    ...
    .concat(
      str.slice(
        str.indexOf(value) >= 0 ? str.indexOf(value) + value.length : 0,
        str.length
      )
    );
}
...
function isPalindrome(str1) {
  ...
  if (
    ...
    str
      .slice(Math.floor(str.length / 2) + (str.length % 2))
      .split('')
      .reverse()
      .join('')
  ) {
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((l) => {
    if (l.length > maxWordLength) {
      maxWordLength = l.length;
      maxWord = l;
    }
  });
  return maxWord;
...
function invertCase(str) {
  ...
  function invert(l) {
    if (!Number.isNaN(l)) {
      return isUpperCase(l) ? l.toLowerCase() : l.toUpperCase();
    }
    return l;
  }
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((x) =>
      alpha.indexOf(x) < 0 ? x : alpha.charAt(alpha.indexOf(x) + 13)
    )
    .join('');
}
